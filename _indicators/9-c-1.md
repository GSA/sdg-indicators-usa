...
comments_and_limitations: >-
  ...
  year's data on 3G+ and 2G+ coverage, in the body of the report or in its
  associated web appendices and "quick facts."  Note that for 2015 and 2016
  data, 2G+ coverage appears simply as total mobile coverage, since "2G or
  faster" captures all digital air interfaces, and (1G) analog service is no
  longer offered.  Note also that since the measures are 3G or faster and 2G or
  faster, it is the gap between the two figures that gives the coverage of 2G
  only, or narrowband, service.
...
time_period: Figures are as of year end.
us_method_of_computation: >-
  For 2015 and 2016,coverage is computed using "Form 477" data reported by
  operators to the FCC, and using the "centroid method."  Population data comes
